...
class FbxMesh extends FbxGeometry {
  ...
  List<Vector3?>? get deformedPoints {
    _deformedPoints ??= List.filled(points!.length, null);
  ...
  void generateClusterMap() {
    clusterMap = List.filled(points!.length, null);
  ...
  void generateDisplayMeshes() {
    ...
    disp.pointMap = List.filled(points!.length, null);
  ...
  void _loadPoints(FbxElement e) {
    ...
    points = List.filled(p.length, null);
  ...
  void _loadNormals(FbxElement e) {
    ...
    for (final c in e.children) {
      ...
      if (c.id == 'MappingInformationType') {
        ...
        normals!.data = List.filled(p.length ~/ 3, null);
        for (var i = 0, j = 0, len = p.length; i < len; i += 3) {
          normals.data[j++] =
              Vector3(toDouble(p[i]), toDouble(p[i + 1]), toDouble(p[i + 2]));
        }
  ...
  void _loadUvs(FbxElement e) {
    ...
    for (final c in e.children) {
      ...
      if (c.id == 'MappingInformationType') {
        ...
        uvs!.data = List.filled(p.length ~/ 2, null);
      ...
      //TODO UVIndex
        ...
        uvs!.indexArray = List.filled(p.length, null);
